...
### 4. Sur macOS, l'application ne démarre pas et affiche une erreur comme "AgenticDub est endommagé et ne peut pas être ouvert."
...
2. Tapez la commande `xattr -cr` suivie d'un espace, puis faites glisser le fichier `AgenticDub.app` de votre fenêtre Finder dans le Terminal. La commande ressemblera à ceci :
    ```
...
Oui, vous pouvez configurer AgenticDub pour utiliser n'importe quel LLM local qui fournit un point de terminaison API compatible avec OpenAI.
...
Non. Actuellement, AgenticDub génère des **sous-titres codés en dur**, ce qui signifie qu'ils sont intégrés directement dans les images de la vidéo. L'application **n'offre pas d'options pour personnaliser le style des sous-titres** ; elle utilise un style prédéfini.

Pour une personnalisation avancée, la solution de contournement recommandée est de :

1. Utiliser AgenticDub pour générer le fichier de sous-titres `.srt` traduit.
...
### 11. J'ai déjà un fichier `.srt` traduit. AgenticDub peut-il l'utiliser pour effectuer uniquement le doublage ?
