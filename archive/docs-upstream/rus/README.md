...
AgenticDub — это универсальное решение для локализации и улучшения аудио и видео, разработанное AgenticDub. Этот минималистичный, но мощный инструмент объединяет перевод видео, дубляж и клонирование голоса, поддерживая как горизонтальный, так и вертикальный форматы, чтобы обеспечить идеальное представление на всех основных платформах (Bilibili, Xiaohongshu, Douyin, WeChat Video, Kuaishou, YouTube, TikTok и др.). С помощью рабочего процесса от начала до конца вы можете преобразовать исходные материалы в красиво готовый кроссплатформенный контент всего за несколько кликов.
...
Вы можете задать вопросы на [Deepwiki AgenticDub](https://deepwiki.com/krillinai/AgenticDub). Он индексирует файлы в репозитории, так что вы можете быстро найти ответы.

### Основные шаги

Сначала загрузите исполняемый файл, соответствующий вашей системе, из раздела [Release](https://github.com/AgenticDub/AgenticDub/releases), затем следуйте приведенному ниже руководству, чтобы выбрать между настольной версией или версией без интерфейса. Поместите загруженное программное обеспечение в пустую папку, так как его запуск создаст некоторые директории, и хранение в пустой папке упростит управление.
...
1. Откройте терминал в директории, где находится исполняемый файл (предполагая, что имя файла AgenticDub_1.0.0_desktop_macOS_arm64)
...
1. Откройте терминал в директории, где находится исполняемый файл (предполагая, что имя файла AgenticDub_1.0.0_macOS_arm64)
2. Выполните следующие команды по порядку:
   ```
...
Развертывание Docker сейчас является legacy / не проверенным путем и сохранено только как справка; пожалуйста, обратитесь к [Инструкциям по развертыванию Docker](./docker.md)
...
[![График истории звезд](https://api.star-history.com/svg?repos=AgenticDub/AgenticDub&type=Date)](https://star-history.com/#AgenticDub/AgenticDub&Date)
