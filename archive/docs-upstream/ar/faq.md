...
### 4. على macOS، التطبيق لا يبدأ ويظهر خطأ مثل "AgenticDub تالف ولا يمكن فتحه."
...
2. اكتب الأمر `xattr -cr` متبوعًا بمسافة، ثم اسحب ملف `AgenticDub.app` من نافذة Finder إلى Terminal. سيبدو الأمر كما يلي:
    ```
...
نعم، يمكنك تكوين AgenticDub لاستخدام أي LLM محلي يوفر نقطة نهاية API متوافقة مع OpenAI.
...
لا. حاليًا، يقوم AgenticDub بإنشاء **ترجمات مشفرة**، مما يعني أنها تُحرق مباشرة في إطارات الفيديو. التطبيق **لا يقدم خيارات لتخصيص نمط الترجمة**؛ يستخدم نمطًا محددًا مسبقًا.

للحصول على تخصيص متقدم، الحل الموصى به هو:

1. استخدام AgenticDub لإنشاء ملف الترجمة `.srt` المترجم.
...
### 11. لدي بالفعل ملف `.srt` مترجم. هل يمكن لـ AgenticDub استخدامه فقط لأداء الدبلجة؟
