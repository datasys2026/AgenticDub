...
### 4. Unter macOS startet die Anwendung nicht und zeigt einen Fehler wie "AgenticDub ist beschädigt und kann nicht geöffnet werden."
...
2. Geben Sie den Befehl `xattr -cr` gefolgt von einem Leerzeichen ein, und ziehen Sie dann die Datei `AgenticDub.app` aus Ihrem Finder-Fenster in das Terminal. Der Befehl sieht dann etwa so aus:
    ```
    xattr -cr /Applications/AgenticDub.app
    ```
...
Ja, Sie können AgenticDub so konfigurieren, dass es jedes lokale LLM verwendet, das einen OpenAI-kompatiblen API-Endpunkt bereitstellt.
...
Nein. Derzeit generiert AgenticDub **hardcodierte Untertitel**, was bedeutet, dass sie direkt in die Videorahmen eingebrannt werden. Die Anwendung **bietet keine Optionen zur Anpassung des Untertitelstils**; sie verwendet einen vordefinierten Stil.

Für eine erweiterte Anpassung ist der empfohlene Workaround:

1. Verwenden Sie AgenticDub, um die übersetzte `.srt`-Untertiteldatei zu generieren.
...
### 11. Ich habe bereits eine übersetzte `.srt`-Datei. Kann AgenticDub sie verwenden, um nur das Synchronisieren durchzuführen?
