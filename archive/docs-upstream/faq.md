...
* This file must be placed inside a `config` folder. The correct structure of the working directory should be:
  ...
  /── config/
  │   └── config.toml
  └── agenticdub.exe(your executable file)
  ```
...
### 4. On macOS, the application won't start and shows an error like "AgenticDub is damaged and can’t be opened."
...
2. Type the command `xattr -cr` followed by a space, then drag the `AgenticDub.app` file from your Finder window into the Terminal. The command will look something like this:
    ```
...
Yes, you can configure AgenticDub to use any local LLM that provides an OpenAI-compatible API endpoint.
...
No. Currently, AgenticDub generates **hardcoded subtitles**, meaning they are burned directly into the video frames. The application **does not offer options to customize the subtitle style**; it uses a preset style.

For advanced customization, the recommended workaround is to:

1. Use AgenticDub to generate the translated `.srt` subtitle file.
...
### 11. I already have a translated `.srt` file. Can AgenticDub use it to just perform the dubbing?
