...
### 4. Trên macOS, ứng dụng không khởi động và hiển thị lỗi như "AgenticDub bị hỏng và không thể mở."
...
2. Gõ lệnh `xattr -cr` theo sau là một khoảng trắng, sau đó kéo tệp `AgenticDub.app` từ cửa sổ Finder của bạn vào Terminal. Lệnh sẽ trông giống như sau:
    ```
...
Có, bạn có thể cấu hình AgenticDub để sử dụng bất kỳ LLM cục bộ nào cung cấp một điểm cuối API tương thích với OpenAI.
...
Không. Hiện tại, AgenticDub tạo ra **phụ đề cứng**, có nghĩa là chúng được đốt trực tiếp vào các khung video. Ứng dụng **không cung cấp tùy chọn để tùy chỉnh kiểu phụ đề**; nó sử dụng một kiểu đã được định sẵn.

Để tùy chỉnh nâng cao, cách làm việc được khuyến nghị là:

1. Sử dụng AgenticDub để tạo tệp phụ đề `.srt` đã dịch.
...
### 11. Tôi đã có một tệp `.srt` đã dịch. AgenticDub có thể sử dụng nó chỉ để thực hiện lồng ghép không?
