...
### 4. No macOS, o aplicativo não inicia e mostra um erro como "AgenticDub está danificado e não pode ser aberto."
...
2. Digite o comando `xattr -cr` seguido de um espaço, e arraste o arquivo `AgenticDub.app` da sua janela do Finder para o Terminal. O comando ficará assim:
    ```
...
Sim, você pode configurar o AgenticDub para usar qualquer LLM local que forneça um endpoint de API compatível com OpenAI.
...
Não. Atualmente, o AgenticDub gera **legendas codificadas**, o que significa que elas são queimadas diretamente nos quadros do vídeo. O aplicativo **não oferece opções para personalizar o estilo das legendas**; ele usa um estilo predefinido.

Para personalização avançada, a solução alternativa recomendada é:

1. Usar o AgenticDub para gerar o arquivo de legenda traduzido `.srt`.
...
### 11. Eu já tenho um arquivo `.srt` traduzido. O AgenticDub pode usá-lo apenas para fazer a dublagem?
