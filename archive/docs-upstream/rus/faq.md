...
### 4. На macOS приложение не запускается и показывает ошибку "AgenticDub поврежден и не может быть открыт."
...
2. Введите команду `xattr -cr`, затем пробел, затем перетащите файл `AgenticDub.app` из вашего окна Finder в Терминал. Команда будет выглядеть примерно так:
    ```
...
Да, вы можете настроить AgenticDub для использования любой локальной LLM, которая предоставляет совместимый с OpenAI API конечный пункт.
...
Нет. В настоящее время AgenticDub генерирует **жестко закодированные субтитры**, что означает, что они встраиваются непосредственно в кадры видео. Приложение **не предлагает опций для настройки стиля субтитров**; оно использует предустановленный стиль.

Для продвинутой настройки рекомендуется следующий обходной путь:

1. Используйте AgenticDub для генерации переведенного файла субтитров `.srt`.
...
### 11. У меня уже есть переведенный файл `.srt`. Может ли AgenticDub использовать его только для дубляжа?
