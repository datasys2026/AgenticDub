...
def clamp_cues(cues, max_duration):
    if not max_duration:
        return cues
    kept = []
    for cue in cues:
        if cue["start"] >= max_duration:
            continue
        item = dict(cue)
        item["end"] = min(item["end"], max_duration)
...
def write_package(video, subtitle, original_subtitle, out_dir, cues, scenes):
    ...
    shutil.copy2(Path(__file__).with_name("subtitle_review_viewer.html"), out_dir / "index.html")


def write_semantic_blocks(out_dir):
    run([
        sys.executable,
        str(Path(__file__).with_name("build_semantic_blocks.py")),
        "--input", str(out_dir / "review_package.json"),
        "--out-dir", str(out_dir / "semantic_blocks"),
    ])
...
def main():
    ...
    parser.add_argument("--no-semantic-blocks", action="store_true")
    ...
    cues = clamp_cues(cues, args.max_duration)
    ...
    if not args.no_semantic_blocks:
        write_semantic_blocks(out_dir)
