...
### 4. En macOS, la aplicación no se inicia y muestra un error como "AgenticDub está dañado y no se puede abrir".
...
2. Escribe el comando `xattr -cr` seguido de un espacio, luego arrastra el archivo `AgenticDub.app` desde tu ventana de Finder a la Terminal. El comando se verá algo así:
    ```
...
Sí, puedes configurar AgenticDub para usar cualquier LLM local que proporcione un punto de API compatible con OpenAI.
...
No. Actualmente, AgenticDub genera **subtítulos codificados**, lo que significa que están grabados directamente en los fotogramas del video. La aplicación **no ofrece opciones para personalizar el estilo de los subtítulos**; utiliza un estilo preestablecido.

Para una personalización avanzada, la solución recomendada es:

1. Usar AgenticDub para generar el archivo de subtítulos `.srt` traducido.
...
### 11. Ya tengo un archivo `.srt` traducido. ¿Puede AgenticDub usarlo solo para realizar el doblaje?
